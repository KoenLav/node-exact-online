...
var Client = function(options) {
  ...
  services.forEach(function(service) {
    try {
      ...
      exports[service] = require("./services/" + service.toLowerCase())[
        service
      ];
...
Client.prototype.checkAuth = function(cb) {
  ...
  if (this.oauth.authorized && this.oauth.expires > Date.now()) {
    ...
    (this.oauth.authorized && this.oauth.expires < Date.now()) ||
    (!this.oauth.authorized && this.oauth.refreshToken)
  ) {
    if (this.options.debug) {
      console.log(
        this.options.prefixDebug +
    ...
    this.authorize(this.oauth.requestCode, function(err, token) {
      cb(err, true, token);
    });
  } else {
    if (this.options.debug) {
...
Client.prototype.authorize = function(code, callback) {
  ...
  this.token(code, "authorization_code", this.options.redirectUri, function(
    err,
    token
  ) {
    if (self.options.debug) {
...
Client.prototype.refreshToken = function(callback) {
  ...
  if (this.options.debug) {
    console.log(
      this.options.prefixDebug +
        '.refreshToken - Refreshing token with refresh_token: "' +
...
Client.prototype.setRefreshToken = function(refreshToken) {
  if (this.options.debug) {
    console.log(
      this.options.prefixDebug +
        '.setRefreshToken - Setting refresh token "' +
        refreshToken +
        '"'
    );
  }
...
Client.prototype.getToken = function(refreshToken) {
  if (this.options.debug) {
    console.log(
      this.options.prefixDebug + '.getToken - Get token "' + refreshToken + '"'
    );
  }
